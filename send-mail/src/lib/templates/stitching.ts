import { getDataStitching} from '../API/dataStitching'

const apiStitching = new getDataStitching();

export async function completeProdTemplateStitching(name: string, url: string) {
...
async function prodTemplate() {
    try {
        const dataFromApi = await apiStitching.fetchDataStitching();
